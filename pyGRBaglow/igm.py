...
def madau(wavelength, redshift, lylim=False, metals=False, Xcut=False, Xlim=100):
    ...
    for n in range(n_transitions_max - 1):
        w = np.where(
            (wavelength <= Ly_wvl[n] * (1 + redshift))
    ...
    if lylim:
        ...
        xm = 1 + redshift
        tau_eff[w] += (
            0.25 * xc ** 3.0 * (xm ** 0.46 - xc ** 0.46)
            + 9.4 * xc ** 1.5 * (xm ** 0.18 - xc ** 0.18)
            - 0.7 * xc ** 3.0 * (xc ** (-1.32) - xm ** (-1.32))
    ...
    w = np.where(trans_igm < 0.0)
    trans_igm[w] = 0.0
    w = np.where(trans_igm > 1.0)
    trans_igm[w] = 1.0
...
def dla(wavelength, redshift_dla, NHI):
    ...
    Lambda_alpha = 3.0 * gu_over_gl ** (-1.0) * f_alpha * Lambda_cl_alpha
    nu_alpha = cc.c_light_m_s / (1216e-10)

    # photon frequency (Hz) in the rest frame
    nu_rf = nus * (1 + redshift_dla)
    sigma_alpha = (
        (3.0 * lambda_alpha ** 2.0 * f_alpha * Lambda_cl_alpha / (8.0 * np.pi))
        * Lambda_alpha
        * (nu_rf / nu_alpha) ** 4.0
        / (
            4.0 * np.pi ** 2.0 * (nu_rf - nu_alpha) ** 2.0
            + Lambda_alpha ** 2.0 * (nu_rf / nu_alpha) ** (6.0) / 4.0
        )
    )
    # ---------------------------------------------------------------------
    ...
    w = np.where(trans_dla < 0.0)
    trans_dla[w] = 0.0
    w = np.where(trans_dla > 1.0)
    trans_dla[w] = 1.0
